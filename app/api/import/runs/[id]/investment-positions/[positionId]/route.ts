...
import type { InvestmentPositionStatus } from '@/lib/ingest/types'

interface RouteParams {
  params: Promise<{ id: string; positionId: string }>
}

const INVESTMENT_POSITION_STATUSES = new Set(['blocked', 'needs_review', 'reviewed', 'ignored'])
...
function parseStatus(body: Record<string, unknown>): InvestmentPositionStatus | null {
  const status = body.status
  if (typeof status !== 'string') return null
  if (!INVESTMENT_POSITION_STATUSES.has(status)) return null
  return status as InvestmentPositionStatus
}
...
function mutationErrorResponse(error: unknown): NextResponse | null {
  ...
  if (error instanceof InvestmentPositionNotFoundError) {
...
export async function PATCH(req: NextRequest, { params }: RouteParams): Promise<NextResponse> {
  ...
  try {
    const result = updateInvestmentPositionStatus({
      importRunId: id,
